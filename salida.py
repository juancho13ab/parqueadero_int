...
def save_exit_to_db(license_plate):
    conn = mysql.connector.connect(
        host="localhost",
        user="root",
        password="",
    ...
    cursor = conn.cursor()
    sql = "UPDATE vehicles SET exit_time = NOW() WHERE licensePlate = %s AND exit_time IS NULL"
    cursor.execute(sql, (license_plate,))
    conn.commit()
    cursor.close()
    conn.close()
...
while True:
    ...
    if plate and is_plate_registered(plate):
        save_exit_to_db(plate)
        print(f"Placa detectada y salida registrada: {plate}")
        break
